            ...
            <div className="relative z-10 space-y-6">
              ...
              {selectedTemplate === 'exec' && (
                <div className="space-y-4 text-xs leading-relaxed text-slate-800 font-sans">
                  ...
                  <p>
                    Based on the active state cohort selection, the aggregate estimated licensing fees sum to <strong>${activeStates.reduce((acc, s) => { if (clarityEnacted && s.nmlsRequired) return acc; if (s.abbreviation === 'NY' && spdiEquivalence) return acc; if (s.estCost) { const parsed = parseInt(s.estCost.replace(/[^0-9]/g, '')); return acc + (isNaN(parsed) ? 15000 : parsed * (s.estCost.includes('K') ? 1000 : 1)); } return acc + 15000; }, 0).toLocaleString()}</strong>. Combined surety bond collateral requirements require a minimum escrow allocation of <strong>${activeStates.reduce((acc, s) => { if (clarityEnacted && s.nmlsRequired) return acc; if (s.abbreviation === 'NY' && spdiEquivalence) return acc; if (s.suretyBond) { const val = parseInt(s.suretyBond.replace(/[^0-9]/g, '')) || 0; return acc + val; } return acc; }, 0).toLocaleString()}</strong>.
                  </p>
                </div>
              )}
